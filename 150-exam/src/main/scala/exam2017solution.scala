...
object Q1 { // Association lists
  ...
  // reduce for lists (apparently it has, but I discovered aftersolving, and it
...
object Q5 {
  ...
  def flatten2[A] (s: =>Stream[List[A]]) :Stream[A] = s match {
    case Empty => empty
    case Cons(h,t) => h() match {
      case Nil => flatten2 (t())
      case (lh::lt) => cons (lh, flatten2 (Cons(()=>lt,t)) )
    }
  }
